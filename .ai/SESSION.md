...
chore [CLD/Fable] 마켓핏랩 길드 바운티 온보딩 — 맥미니 1인 1에이전트 인프라 바운티 참여 확정(글로벌 메모리 project_marketfitlab-agent-bounty 등재, AIZen 자산 openclaw-setup·secrets·launchd·승인게이트 재활용 예정). Tailscale mfitlab.com tailnet 가입 완료(soomshuim@gmail.com, 관리자 승인 대기 상태). SSH 공개키는 신규 발급 없이 맥북프로 기존 키(~/.ssh/id_ed25519.pub, GitHub soomshuim 등록 키) 재사용 — .ssh 생성일 실측으로 이 맥 출생 확인, 판매한 구 맥미니는 초기화 후 양도 확인. .pub 파일로 관리자 전달 완료. AIZen 코드 변경 없음.
