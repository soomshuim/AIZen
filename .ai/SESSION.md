...
docs [CLD/Fable] 2026-09-02 HKG 심화 인터뷰 질문지 작성·발송 완료 — ① 모니카·니키 인터뷰 가이드 신설(docs/hkg-bounty/v1-interview-{monica,nikki}.md): 판별 축 3종(ⓐ리터러시/ⓑ귀찮음/ⓒ길드 토큰) + 모니카 퀘스트봇 MCP 인지('안 만들어도 될 일' 판별) + 판별→방향 매트릭스 + 재질문 금지 가드(니키 주기·채널 실명·B-0, 피저빌리티 비노출). ② 전제 점검(젠 지시): 소스에 없는 전제 4건 교정(니키 A3·C1, 모니카 C3·D1). 니키 '클로드코드 반자동화' 출처 검증 — 회신 요약(v1-p0-nikki B1-4)이 유일 출처, 원문 레포 부재, 싱크 전사엔 반대 신호('써봤는지 확인 필요') → 젠 판정 '응답 우선'으로 시도 인정·유지. ③ 슬랙 DM 발송판 → 젠 취사 2건(니키 C2 길드장뉴스=젠 직접 확보, 모니카 C1·C2 채널 목록=젠 기확보·후속질문 발송불요 종결) → 젠 리라이트 최종본 발송(모니카 6문·니키 5문), 문서를 발송 원문 기준 갱신 + 변경점 기록. 주목: 니키 대외 톤 '진행'→'검토중' 완화(내부 확정 불변), 모니카 C3 '(어제 등록한 퀘스트 말고)' 정밀화. 회신 대기 = 판별 착수 신호. 니키 B2(팀 비용)·C3(성공 계측)는 DM 미발송(대면용). 아티팩트 2건 발행·교정 루프 3회. 커밋 2953a0c~0f66604

docs [CLD/Fable] 2026-08-31 HKG 설문 2 회신 접수·P0 정리 + 젠·도 싱크 회의록 확정 — ① 모니카·니키 회신 분석 → docs/hkg-bounty/v1-p0-monica.md·v1-p0-nikki.md 신설. 니키 봇 = 주간 다이제스트 확정(젠 기준: 범용성·구현 난이도, 믹스패널 봇은 차순위 보류), 주기 매주 확정, 니키 채널명 미표시 원인 = 젠 계정 권한(비공개 채널 미가입) 실측. ② 젠·도 싱크(27분) 녹음→개인 드라이브 업로드→전사→정식 회의록 확정(ff513b7) — 개인판 회의록 체인 첫 정식 가동: 격리 정확성 검증(발견 12: 확실 5 수정·애매 7 → 웹 리뷰 라운드 1 코멘트 9건으로 전부 해소)→웹 드래그 교정 2라운드→확정·저장·PDF 데스크톱 복사. ③ 싱크 결정: 심화 인터뷰 선행(모니카 채널 DM·니키 재문의 모두 인터뷰로 흡수), 퀘스트봇 MCP 기존재 발견 — 모니카 케이스는 MCP 안내만으로 풀릴 가능성(안 만들어도 될 일 후보). ④ 인프라(agent-center 06a0910): 개인판 meeting-notes 글로벌 래퍼 신설 + record-meeting §2b 재배선 — 동일성 원칙 = 예외 딱 2종(슬랙 무발송·회사 레포 커밋 금지), 그 외 회사판 전 단계 동일. plumlabs 커밋 사고 1건(개인 건 회사 레포 커밋) → 그대로 두기 종결 + 재발 방지 메모리 등재.

docs [CLD/Fable] 2026-08-30 HKG 설문 2 발송 완료 — 젠이 리라이트한 최종 DM을 모니카·니키에게 직접 발송, docs/hkg-bounty/v1-p0-questions.md를 발송 원문 기준으로 갱신(초안은 git 이력 cfbaf3e). 리라이트 변경점: 호칭·이모지 톤 완화 / 모니카 Q1 퀘스트봇 명령어·플로우 전제로 좁힘(입력처=퀘스트봇 신호) / 모니카 채널 목록 꼬리 질문 제거(회신 후 후속 질문 필요). 글로벌 메모리 project_marketfitlab-agent-bounty 현행화. 다음 = 회신 대기, 회신 수신이 P0 확정 착수 신호.

docs [CLD/Fable] 2026-08-30 HKG 접수 마감 결과 + 설문 2 초안 — 접수 2건: [개인] 모니카(슬랙 채널·DM·이메일·캘린더 참조 퀘스트 자동 입력, 시간만 본인 입력) / [사업부] 솔루션즈·니키(① 고객사 믹스패널 사용 현황 리포트 봇 ② 일일 업무 다이제스트 봇, 2후보). 젠 결정: 3안 중 1안 — 두 케이스 진행, 케이스 간 우선순위 선별 없음(재모집·드랍 기각). 설문 2 = P0 확정 질문지 초안 작성 docs/hkg-bounty/v1-p0-questions.md — 모니카 7문항, 니키 B-0(두 봇 중 선택)+봇별 4문항, DM 접근 제약 대비 폴백 질문(A-4) 포함, 피저빌리티(2주) 기준 비노출 유지(2026-08-25 결정 준수). 발송 전 젠 리라이트 대기. 미신청 사유 5건 실측 수집(양보/자가구현 인식/부담/글 길어서 안 봄/고객사 입장 같음) — 글로벌 메모리 project_marketfitlab-agent-bounty 현행화. 리스크 승격: 슬랙·노션·구글 인티그레이션 허용 확인 주체 미배정 — 두 케이스 모두 회사 슬랙 읽기 필요, 모니카는 지메일·캘린더도 필요, 폴·진배 9/8까지 통신 불가.
